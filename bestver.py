...
class NeoCortex:
    ...
    def __init__(self, model_name: str = "deepseek/deepseek-chat-v3", temperature: float = 0.2):
        ...
        # Updated API key
        self.api_key = "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"
    ...
    def _generate_response(self, prompt: str, emit_thoughts=False) -> str:
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Add enhanced system prompt that instructs the model to share its reasoning
                system_prompt = """You are an advanced reasoning AI assistant. Be concise and efficient.
                
                IMPORTANT: When solving a complex problem or reasoning about a challenging question,
                explicitly share your thought process as you analyze it. Start lines with 'Thinking:'
                when you're working through your reasoning, then provide your final answer.
                ...
                For complex problems, make sure to provide thorough, complete solutions that don't cut off mid-answer.
                If providing code, ensure all code blocks are properly closed.
                """
    ...
    def _generate_completion(self, truncated_content: str, original_prompt: str) -> str:
        ...
        cache_key = f"completion_{original_prompt}_{self.model_name}_{self.temperature}"
        if cache_key in self.response_cache:
            return self.response_cache[cache_key]
    ...
    def solve(self, query: str, show_work: bool = True) -> Dict:
        ...
        combined_response = self._generate_response(combined_prompt, emit_thoughts=True)
        ...
        solution_response = self._generate_response(solution_prompt, emit_thoughts=True)
        ...
        verification_response = self._generate_response(verification_prompt, emit_thoughts=True)
...
API_KEY = "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"
...
class ModelSettingsWidget(QWidget):
    ...
    def init_ui(self):
        ...
        api_key_display = QLineEdit("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")
